...
- **A switching-rate difference between arms needs the response-magnitude control** (`02e`) before it is called a splicing phenotype. The UT deficit does not survive it (crude OR 0.51 → MH 0.77, p = 0.11); the HT one does (2.29 → 2.49). **But the HT contrast is currently invalid for a separate reason** — see the `H_HT` rule below.
